...
def update_post(post_id: UUID, post_update: PostCreate, db: Session, current_user: User = Depends(get_current_user)):
...
def delete_post(post_id: UUID, db: Session, current_user: User = Depends(get_current_user)):
    post = get_post(post_id, db)
    if post.user_id != current_user.id:
